...
public enum Encode {
    ...
    public static Encode getEnum(String str) {
        ...
        try {
            Charset cset = Charset.forName(str);// get Charset from argument string
            String charsetname = cset.name();// get Charset name

            //
            for (Encode enumStr : enumArray) {
                // compare each enum value with charset name
